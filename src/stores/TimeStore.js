...
export default class TimeStore extends Store {
    ...
    tick() {
        this.time += 1;
        this.hours =
            Math.floor(this.time / 3600) < 10 ? `0${Math.floor(this.time / 3600)}` : Math.floor(this.time / 3600);
        this.minutes =
            Math.floor(this.time / 60) % 60 < 10
                ? `0${Math.floor(this.time / 60) % 60}`
                : Math.floor(this.time / 60) % 60;
        this.seconds = this.time % 60 < 10 ? `0${this.time % 60}` : this.time % 60;
    ...
    setTick() {
        ...
        this.setTimerMinutes =
            Math.floor(this.setTime / 60) % 60 < 10
                ? `${Math.floor(this.setTime / 60) % 60}`
                : Math.floor(this.setTime / 60) % 60;
        this.setTimerSeconds = this.setTime % 60 < 10 ? `0${this.setTime % 60}` : this.setTime % 60;
