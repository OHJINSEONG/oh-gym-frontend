    ...
    useEffect(() => {
        ...
        stompClient.current.connect({}, () => {
            stompClient.current.subscribe(`/sub/chats/room/${roomId}`, (chats) => {
                ...
                chatList.map((chat) => {
                    const { id, status, time, message, writer } = chat;
            ...
            stompClient.current.subscribe(`/sub/chat/room/${roomId}`, (chat) => {
                const { id, status, time, message, writer } = JSON.parse(chat.body);
            ...
            stompClient.current.subscribe('/sub/user/chat', (chat) => {
                const { userName, trainerName, trainerImage } = JSON.parse(chat.body);
